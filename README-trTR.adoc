...
ifdef::env-github[]
TIP: https://rubystyle.guide adresinde bu rehberin daha gelişmiş sayfalamaya sahip güzel bir versiyonunu bulabilirsiniz.
endif::[]
...
Burada gösterilen yönergeler, yazılan kodun okunabilirliğini artırmak ve geniş Ruby kodu yelpazesinde tutarlılığı sağlamayı amaçlamaktadır. Aynı zamanda rastgele bir ideal yerine Ruby'nin gerçek dünyadaki kullanımını yansıtmayı amaçlamıştır.
...
Ruby topluluğunda belirli kurallarla ilgili bir fikir birliğininin sağlanamadığı alanlar vardır (string tırnakları, hash ifadelerindeki boşluklar, çok satırlı metot zincirindeki nokta pozisyonu gibi). Bu senaryolarda tüm kullanımlar kabul edilir ve birini seçip tutarlı bir şekilde uygulamak size kalmıştır.
...
Ruby yazılımcı mutluluğu için optimize edilmiştir. Bu rehberin de maksimum yazılmıcı mutluluğuna yardım edeceğine inanıyoruz.
...
* İlgili yeri çevreleyen kod ile tutarlı olunması.
...
TIP: UTF-8, Ruby 2.0 versiyonundan beri varsayılan karakter kodlamasıdır.
...
TIP: Çoğu editör ve IDE bu konuda size yardımcı olacak konfigürasyon seçeneklerine sahiptir.
Genellikle sınırı aşan satırları vurgularlar.

.Modern Geniş Ekran Monitörlerin Dünyasında Neden 80 Karakter Kullanalım? 
****

Bugünlerde bir çok insan maksimum 80 karakterlik satır uzunluğunun geçmişten
gelen bir alışkanlık olduğunu ve artık bir anlam ifade etmediğini düşünüyor.
Artık modern ekranlar tek bir satıra 200'den fazla karakter sığdırabiliyor.
Yine de daha kısa satır uzunluğu kullanmanın kazandıracağı takım önemli faydalar
vardır.

Birincisi ve en önemlisi - çok sayıda araştırma insanların dikey olarak daha hızlı
okuduğunu ve uzun satırların okumayı zorlaştırdığını göstermiştir. Daha önce de
belirtildiği üzere bu rehberin amaçlarından biri yazdığımız kodun daha kolay
okunmasını sağlamaktır.

Ek olarak, kullandığınız editör penceresinin genişliğini sınırlamak, birden fazla
dosyayı yan yana açmayı mümkün kılar ve kodun farklı sürümlerini gösteren inceleme
araçlarını kullanırken daha kolay çalışmayı sağlar.

Çoğu araçtaki varsayılan düzenleme, kodun görsel yapısını bozarak anlaşılmasını
zorlaştırır. Sınırlar, satırları kaydırırken araç son sütuna bir işaret glifi
eklese bile, pencere genişliği 80'e ayarlanmış editörlerde kaydırmayı önlemek
için seçilir. Bazı web tabanlı araçlar dinamik satır kaydırma seçeneği sunmayabilir.

Bazı takımlar özellikle daha geniş satır uzunlukları tercih eder. Öncelikli olarak
bu konuda takımla anlaşmaya varmak şartıyla, satır uzunluğunu 100 karaktere veya
120 karaktere kadar artırmak uygundur. Lütfen 120 karakterden daha fazla satır
uzunluğu kullanma isteğinizi dizginleyin.
****
...
TIP: Çoğu editör ve IDE satır sonundaki boşlukları gösterme ve kaydedildiğinde otomatik 
olarak silme seçeneğine sahiptir.
...
TIP: Bu işlem elle değil editör tarafından otomatik yapılmalı.
...
----
# kötü
puts 'foobar'; # geresiz noktalı virgül
...
.Biraz Tarih
****
Bu hem "The Ruby Programming Language" hem de "Programming Ruby"de kullanılan stildir.
Tarihsel olarak `case` ve `switch` ifadelerinin blok olmadığı, dolayısıyla girintili olmaması,
`when` ve `else` anahtar kelimelerinin birer etiket olması gerçeğinden türetilmiştir.
****

=== Koşullu Atama Girintisi [[indent-conditional-assignment]]

Bir değişkene koşullu ifadenin sonucunu atarken, girintileri aynı hizada tutun.

[source,ruby]
----
# kötü - oldukça karışık
kind = case year
...
# iyi (ve genişlik açısından daha verimli)
kind =
  case year
  when 1850..1889 then 'Blues'
  when 1890..1909 then 'Ragtime'
...
=== Metotlar Arası Boş Satırlar [[empty-lines-between-methods]]

Metot tanımları arasında ve metot içindeki kodları mantıksal paragraflara ayırmak için boş satırlar kullanın.
...
=== İki veya Daha Fazla Boş Satır [[two-or-more-empty-lines]]

Arka arkaya birden fazla boş satır kullanmayın.

[source,ruby]
----
# kötü - İki tane boş satır var.
some_method
...
=== Nitelik Erişimcisi Etrafındaki Boş Satırlar [[empty-lines-around-attribute-accessor]]

Nitelik erişimcisi (attribute accessor) etrafında boş satır kullanın.
...
=== Erişim Belirteçleri Etrafındaki Boş Satırlar [[empty-lines-around-access-modifier]]

Erişim belirteçleri etrafında boş satır kullanın.
...
=== Gövdeler Etrafındaki Boş Satırlar [[empty-lines-around-bodies]]

Metot, sınıf, modül ve blok gövdeleri etrafında boş satır kullanmayın.
...
=== Metot Argümanlarının Sonunda Virgül [[no-trailing-params-comma]]

Bir metot çağrısı yaparken son parametreden sonra virgül kullanmaktan kaçının. Özellikle parametreler
ayrı satırlarda değilse.

[source,ruby]
----
# kötü - taşıma/ekleme/silme daha kolay ama yine de tercih edilmez
some_method(
  size,
  count,
  color,
...
=== Eşitliklerin Etrafındaki Boşluklar [[spaces-around-equals]]

Metot parametrelerine varsayılan değer atarken `=` operatörünün etrafında boşluk kullanın.
...
Birkaç Ruby kitabı ilk kullanımı önerir. Ancak ikincisi daha okunabilirdir.

=== İfadelerde Satır Devamlılığı [[no-trailing-backslash]]

Gerekli olmadıkça `\` ile satır bölmekten kaçının.
Pratikte, string birleştirme dışında herhangi bir şey için satır devamlarını kullanmaktan kaçının.

[source,ruby]
----
# kötü (gereksiz '\' kullanımı)
result = 1 - \
         2

# bad (\ gerekli, ancak korkunç derecede çirkin)
result = 1 \
         - 2
...
=== Çok Satırlı Metot Zincirleri [[consistent-multi-line-chains]]

Birden fazla satırda devam eden metot zincirlerinde tutarlı olun.
Ruby topluluğunda kabul ikisi de iyi olarak görülen iki kullanım vardır - başında ve sonunda `.` kullanımı.

==== `.` Başta [[leading-dot-in-multi-line-chains]]

Başka bir satırda metot zincirine devam ederken `.` işaretini ikinci satırda kullanın.

[source,ruby]
----
# kötü - ikinci satırı anlamak için ilk satırı okumanız gerekir
one.two.three.
  four

# iyi - ikinci satırda ne olduğu hemen anlaşılıyor
one.two.three
  .four
----

==== `.` Sonda [[trailing-dot-in-multi-line-chains]]

Başka bir satırda metot zincirine devam ederken, ifadenin devam ettiğini belirtmek için
ilk satıra `.` işaretini ekleyin.

[source,ruby]
----
# kötü - zincirin devam ettiğini görmek için ikinci satırı okumanız gerekir
one.two.three
  .four

# good - zincirin devam ettiği ilk satırdan anlaşılıyor
one.two.three.
  four
----

Her iki kullanım hakkındaki tartışmayı https://github.com/rubocop/ruby-style-guide/pull/176[burada] bulabilirsiniz.

=== Metot Argümanlarını Hizalama [[no-double-indent]]

Birden fazla satırda yazılıyorsa metodun argümanlarını hizalayın.
Satır uzunluğu sınırı nedeniyle argümanları hizalamak mümkün olmadığında, ilk argümandan sonra
tek girinti de kabul edilebilir.

[source,ruby]
----
# başlangıç (satır çok uzun)
...
=== Parametre Olarak Hash (Implicit Options Hash) [[no-braces-opts-hash]]

IMPORTANT: Ruby 2.7'den itibaren parametre olarak verilen hash objelerinin 
etrafındaki parantezler artık isteğe bağlı değildir.

Parametre olarak kullanılan hash objelerinin etrafındaki parantezleri atlayın.
...
=== DSL Metot Çağrıları [[no-dsl-decorating]]

Bir alana ait olan (DSL - Domain Specific Language) metodlar için parantezleri atlayın..
...
=== Metot Çağrılarındaki Boşluklar [[parens-no-spaces]]

Metot adı ve parantezleri arasına boşluk bırakmayın.
...
=== Parantez Erişimlerindeki Boşluk

Bir nesne ve parantezleri arasında boşluk bırakmayın.
...
=== Çok Satırlı Dizileri Hizalama [[align-multiline-arrays]]

Birden çok satırda yazılan dizi elemanlarını hizalayın.
